...
import { authorizePortalPhoto } from "@/modules/portal/portal-access";
...
export async function GET(
  ...
  const photo = await authorizePortalPhoto(id);
  if (!photo) {
    return new NextResponse("Not found", { status: 404 });
  }

  const variant =
    request.nextUrl.searchParams.get("variant") === "preview" ? "preview" : "thumb";
  const relativePath =
    variant === "preview" ? photo.previewPath || photo.thumbPath : photo.thumbPath;
